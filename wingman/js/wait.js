jQuery(document).ready(function($){
    ...
    var getWalmartProducts = function(html){      
        ...
        var walmart_titles = [];
        $(html).find('#searchProductResult .search-result-gridview-items li').find('.search-result-productimage').each(function(){
            var title = $(this).find('img').attr('alt');
            var str = title.replace(/[^a-z0-9\s]/gi, '').replace(/[_\s]/g, ' ');
            var four_word = str.split(/\s+/).slice(0,5).join(" ");
            walmart_titles.push(four_word);
            //console.log('walmart_titles', walmart_titles);            
        });    

       
        if(walmart_titles.length == 0){
            $(html).find('#searchProductResult .search-result-listview-items').find('div').each(function(){                   
                var title =  $(this).find('img').attr('alt');
        ...
        $.unique(walmart_titles);

        //console.log('End Loop');
        localStorage.setItem('walmart_titles', walmart_titles);
        console.log('data',localStorage.getItem('walmart_titles'))
        location.href = chrome.extension.getURL("html/popup.html?walmart=search");
        return;
       ...
       location.href = chrome.extension.getURL("html/popup.html");
       return;
