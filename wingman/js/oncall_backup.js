 ...
 if(message.method == 'openpopup'){   	
	...
	chrome.storage.sync.get(function(data) {
		if(data.userId && data){
			...
			var html = '<div class="modal fade"  class="ui-widget-content" id="myModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">';
				...
				html += '<ul class="nav nav-pills"> <li class="active"><a class="btn btn-info" data-toggle="pill" href="#home">Product List(s)</a></li><li><a class="btn btn-info" data-toggle="pill" href="#menu1">Wing Man</a></li><li style="display:none;"><a type="button" class="btn btn-info" id="hide_popup">Hide</a></li></ul>';
   ...
   //  		var trainindIdArray = trainindIdArrays.filter(function(itm, i, a) {
			...
			arrayOfArrays.forEach( function( el, i ) {   // for each item in the data chain
		        p = p.then( function() {        // after the last action is done, make a new $.ajax
		            return $.ajax( {
		             ...
		             p.then( function() {                    // when all the actions are done
			            $( '#popuptable').fadeOut();    // when all done - fade out
			         });
